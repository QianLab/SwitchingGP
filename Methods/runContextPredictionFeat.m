...
freq = 1000/50; % ms
feature_sets_indices = [1, 41, 81, 121, 161, 201, 240, 266, 345, 424, 503, 562];
...
feature_sets = cell(length(feature_sets_indices)-1, 1);
for f = 1 : length(feature_sets_indices)-1
	feature_sets{f} = feature_sets_indices(f) : feature_sets_indices(f+1)-1;
end
...
gp_model = importdata('../Results/MTGP/gp_model.mat');
...
for i = 1 : length(signals_train)
	...
	[predicted_context, alpha_prob] = predictContextFeat(signal, time, model, pred_lag, max_duration, feature_sets);
