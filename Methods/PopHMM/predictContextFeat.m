function [context_pred, alpha_prob] = predictContextFeat(signal, time, model, pred_lag, max_duration, feature_subsets)
...
feature_set_index = 1 : length(feature_subsets);

% Initial alpha and beta
t = 1;
d = 1;
log_alpha(t, :, d) = -emissionMTGPFeat(signal(t, :), time(t), feature_model, gp_model, feature_set_index, feature_subsets);
...
for t = 2 : time_length
	...
	for d = 1 : D
		...
		% Emission
		log_emission = -emissionMTGPFeat(signal(t_min_d_p1 : t, :), time(t_min_d_p1 : t), feature_model, gp_model, feature_set_index, feature_subsets);
