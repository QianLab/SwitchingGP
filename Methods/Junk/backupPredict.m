function [context_pred, log_prob] = predictContext(signal, time, model, pred_lag, max_duration)
...
% 		emissions(:, d) = -emissionMTGP(signal(t_min_d_p1 : t, :), time(t_min_d_p1 : t), ...
...
% 					end
% 					
% 					log_sum = brml.logsumexp([log_sum; log(p_trans(j, i)) + gamma_is(i, d_prime) + gamma_js(j, d) ...
% 						+ emissions(j, d) + log_alpha_tilde]);
% 				end
% 			end
% 			log_alpha(t, j, d) = log_sum;
% 		end
% 	end
% 	
% 	% Backward steps
...
% 		
% 		% Emission
% 		log_emission = -emissionMTGP(signal(t_min_d_p1 : t, :), time(t_min_d_p1 : t), ...
% 			feature_model, gp_model);
% 		
% 		for j = 1 : num_contexts
% 			% Sum variable
